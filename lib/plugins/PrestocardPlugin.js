...
class PrestocardPlugin extends BasePlugin {
  constructor(browser, logger, pluginArgs) {
    ...
    if (!this.pluginArgs.username) {
      throw new Error(
        'You do not appear to have the "username" key set in your config file for the Prestocard plugin.',
      );
    }

    if (!this.pluginArgs.password) {
      throw new Error(
        'You do not appear to have the "password" key set in your config file for the Prestocard plugin.',
      );
    }
  ...
  async retrievePrestoCSRFToken(args) {
    ...
    await this.page.setContent(tResult, {waitUntil: 'networkidle0'});
  ...
  async scrapeTransactions() {
    ...
    if (tResult !== 'success') {
      throw new Error(
        `There appears to be an error logging in to your Prestocard account. Login result: ${JSON.stringify(
          loginResult,
        )}`,
      );
    }
  ...
  async parseTransactionRows(rawResult) {
    ...
    await this.page.setContent(rawResult, {waitUntil: 'networkidle0'});
