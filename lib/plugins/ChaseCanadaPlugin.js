...
class ChaseCanadaPlugin extends BasePlugin {
  ...
  async scrapeTransactions() {
    ...
    await this.page.goto('https://online.chasecanada.ca/ChaseCanada_Consumer/Login.do', {waitUntil: 'networkidle0'});

    // Fill in the username
    await this.page.type('input[name="username"]', this.pluginArgs.username, {delay: 30});

    // Fill in the password
    await this.page.type('input[name="password"]', this.pluginArgs.password, {delay: 30});

    // Click "Sign On"
    await this.pageClick(
      'body > table > tbody > tr > td > table:nth-child(1) > tbody > tr > td:nth-child(1) > table > tbody > tr:nth-child(10) > td > table > tbody > tr:nth-child(4) > td > input[type="submit"]',
    );

    // Fill in the answer to one of the security questions, if presented
    const securityAnswerFieldSelector = 'form[name="secondaryUserAuthForm"] input[name="hintanswer"]';
    const securityAnswerField = await this.page.$(securityAnswerFieldSelector);
    if (securityAnswerField) {
      await this.page.type(securityAnswerFieldSelector, this.pluginArgs.securityAnswer, {delay: 30});
      await this.pageClick('input[type="submit"][name="submitNext"]');
    }
    ...
    if (!isLoggedIn) {
      ...
      throw new Error(
        'You do not appear to be logged in. Verify your logged in status by looking at screenshot.pdf. Please file a bug report if this is the result of a program error.',
      );
    }
    this.logger.debug('Successfully logged in');

    // Get the current balance
    const currentBalanceRowSelector =
      'body > table > tbody > tr:nth-child(2) > td.sideTable > table > tbody > tr:nth-child(14) > td > table > tbody > tr:nth-child(2) td';
    ...
    for (let stmt of availableStatements) {
      ...
      await this.page.evaluate(
        ...
        'form[name="transHistoryForm"] select[name="cycleDate"]',
        stmt,
      );
      await this.pageClick('form[name="transHistoryForm"] input[type="submit"]');
  ...
  async parseTransactionRows() {
    ...
    const transactionTableSelector =
      'body > table > tbody > tr:nth-child(2) > td:nth-child(2) > table > tbody > tr:nth-child(5) > td:nth-child(2) > table:nth-child(3) > tbody > tr.none > td > table > tbody > tr';

    const {transactionList, mostRecentDateWeveSeen} = await this.page.evaluate(
      (sel, configuredMostRecentTransactionDate, updatedMostRecentTransactionDate) => {
        let transactionList = [];
      ...
      this.configuredMostRecentTransactionDate,
      this.updatedMostRecentTransactionDate,
    );
