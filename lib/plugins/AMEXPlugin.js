...
class AMEXPlugin extends BasePlugin {
  ...
  async scrapeTransactions() {
    ...
    await this.page.goto(
      'https://global.americanexpress.com/myca/logon/canlac/action/LogonHandler?request_type=LogonHandler&Face=en_CA&inav=ca_utility_login',
      {waitUntil: 'networkidle0'},
    );

    // Fill in the username
    await this.page.type('input[id="lilo_userName"]', this.pluginArgs.username, {delay: 30});

    // Fill in the password
    await this.page.type('input[id="lilo_password"]', this.pluginArgs.password, {delay: 30});

    // Click "Log In"
    await this.pageClick('input[id="lilo_formSubmit"]');
    this.logger.debug('Successfully logged in');

    // Simulate clicking "Your Statement" -> "View Statement Activity"
    await this.page.goto(
      'https://global.americanexpress.com/myca/intl/istatement/canlac/statement.do?Face=en_CA&method=displayStatement',
      {waitUntil: 'networkidle0'},
    );
    ...
    await this.pageClick('#daterange', false);
    await this.pageClick('#dateRangeContainer > div.filterMenu div[title="Year to Date"]', false);
  ...
  async parseTransactionRows() {
    ...
    const {transactionList, mostRecentDateWeveSeen} = await this.page.evaluate(
      ...
      'table[id="transaction-table"] > tbody > tr',
