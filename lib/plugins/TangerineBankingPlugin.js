...
class TangerineBankingPlugin extends BasePlugin {
  ...
  async scrapeTransactions() {
    ...
    }, {timeout: 5000}, accountNameSelector);
    await this.page.click(accountNameSelector);
    await this.page.waitForNavigation({waitUntil: 'networkidle'});

    // Get the current balance
    const currentBalanceSelector = '.c-account-details__container .c-account-details__balance';
    ...
    }, {timeout: 5000}, currentBalanceSelector);
    const currentBalance = await this.page.$eval(currentBalanceSelector, (el) => el.innerText.trim());
