...
class TangerineBankingPlugin extends BasePlugin {
  ...
  async scrapeTransactions() {
    ...
    while (true) { // eslint-disable-line no-constant-condition
      let currentPage;
      try {
        currentPage = await page.$eval('#transactionTable > ul > li.active a', (el) => el.innerText.trim());
      } catch (error) {
        this.logger.debug(`Non-fatal error while looking up current page. ${error}`);
        currentPage = '1'; // symbolizes "page 1" as there are no more pages
      }
  ...
  async parseTransactionRows(page, currentPageNumber) {
    ...
    const {transactionList, mostRecentDateWeveSeen} = await page.evaluate((sel, configuredMostRecentTransactionDate, updatedMostRecentTransactionDate) => {
      ...
      rows.forEach((row) => {
        ...
        const amount = eles[3].innerText.trim();
